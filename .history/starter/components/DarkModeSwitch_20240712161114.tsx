...
export default function DarkModeSwitch() {
  ...
  return (
    <div>
      {currentTheme === "dark" ? (
        <IconWithProps IconComponent={MdBrightness5} className="cursor-pointer" onClick={handleToggleTheme} />
      ) : (
        <IconWithProps IconComponent={MdBrightness3} className="cursor-pointer" onClick={handleToggleTheme} />
      )}
    </div>
  );
}
