...
export default function DarkModeSwitch() {
  ...
  return (
    <div>
      {currentTheme === "dark" ? (
        <IconWithClassName IconComponent={MdBrightness5} className="cursor-pointer hover:text" />
      ) : (
        <IconWithClassName IconComponent={MdBrightness3} className="cursor-pointer hover:text" />
      )}
    </div>
  );
}
