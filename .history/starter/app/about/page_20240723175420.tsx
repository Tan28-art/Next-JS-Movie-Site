import React from "react";

export default function page() {
  return (
    <div>
      <h1 className="text-2xl font-medium text-emerald-500">About Page</h1>
      <p className="text-xl">
        Welcome to IMDb Clone, your go-to source for all things movies! Our goal
        is to provide a simple, easy-to-use platform where film enthusiasts can
        discover and explore a wide range of films.
      </p>
      
        <p>Who We Are</p> <p>IMDb Clone was created by one person who
        wanted to share their passion for cinema. I aim to make finding and
        learning about movies enjoyable and accessible for everyone.
      </p>
      <p>
        <p>What We Offer</p> <p></p>At IMDb Clone, we provide a growing database of movies
        across various genres and eras.
      </p>
    </div>
  );
}
